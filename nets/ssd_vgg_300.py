...
class SSDNet(object):
    ...
    default_params = SSDParams(
        ...
        feat_shapes=[(28, 28), (14, 14), (7, 7), (4, 4), (2, 2), (1, 1)],
        ...
        anchor_ratios=[[2, .5],
                       ...
                       [2, .5],
                       [2, .5]],
        anchor_steps=[8, 16, 32, 56, 112, 224],
        anchor_offset=0.5,
    ...
    def losses(self, logits, localisations,
        ...
        """
        return ssd_losses_old3(logits, localisations,
                          gclasses, glocalisations, gscores,
...
def ssd_net(inputs,
    ...
    with tf.variable_scope(scope, 'ssd_300_vgg', [inputs], reuse=reuse):
        ...
        x = conv2d_same(inputs, 64, 7, 2, scope='conv_init')
        x = batch_norm(x, is_training=is_training, scope='batch_norm_00')
        x = tf.nn.relu(x)
        print("x0 shape = ", x.shape)
        x = tf.layers.max_pooling2d(x, pool_size=3, strides=2, padding='same', name='maxpool_0')
        print("x1 shape = ", x.shape)
        
        end_points['block1'] = x
        ch = 64 # paper is 64
        
        x = conv2d_same(x, ch, 3, 1, scope='conv_init_1')
        print("x2 shape = ", x.shape)
        for i in range(residual_list[0]) :
            x = residual_block(x, ch, is_training=is_training, downsample=False, scope='resblock0_' + str(i))
        ...
        x2 = x
        end_points['block4'] = x2
        end_points['block5'] = x2
        end_points['block6'] = x2   
        x = residual_block(x, channels=ch*4, is_training=is_training, downsample=True, scope='resblock2_0')
        for i in range(1, residual_list[2]) :
            x = residual_block(x, channels=ch*4, is_training=is_training, downsample=False, scope='resblock2_' + str(i))
        print("x 3 shape = ", x.shape)
        x3 = x
        end_points['block7'] = x3
        x = residual_block(x, channels=ch*8, is_training=is_training, downsample=True, scope='resblock_3_0')
        for i in range(1, residual_list[3]) :
            x = residual_block(x, channels=ch*8, is_training=is_training, downsample=False, scope='resblock_3_' + str(i))
        print("x 4 shape = ", x.shape)
        x4 = x
    
        conv8_1 = conv2d_same(x4, 128, 1, stride=1, scope='conv8_1') 
        conv8_1 = tf.nn.relu(conv8_1)
        conv8_2 = conv2d_same(conv8_1, 128, 3, stride=1, scope='conv8_2')
        conv8_2 = tf.nn.relu(conv8_2)
        print("conv8_2 shape = ", conv8_2.shape)
        end_points['block8'] = conv8_2
        # block9 then output is 5*5*256
        conv9_1 = conv2d_same(conv8_2, 128, 1, stride=1, scope='conv9_1')
        conv9_1 = tf.nn.relu(conv9_1)
        conv9_2 = conv2d_same(conv9_1, 128, 3, stride=2, scope='conv9_2')
        conv9_2 = tf.nn.relu(conv9_2)
        end_points['block9'] = conv9_2
        print("conv9_2 shape = ", conv9_2.shape)
        # block10 then output is 3*3*256
        conv10_1 = conv2d_same(conv9_2, 128, 1, stride=1, scope='conv10_1')
        conv10_1 = tf.nn.relu(conv10_1)
        conv10_2 = conv2d_same(conv10_1, 128, 3, stride=2, scope='conv10_2')
        conv10_2 = tf.nn.relu(conv10_2)
        end_points['block10'] = conv10_2
        print("conv10_2 shape = ", conv10_2.shape)
        # block11 the output is 1*1*256
        conv11_2 = global_avg_pooling(conv10_2, scope='gag')
        # conv11_1 = slim.conv2d(conv10_2, 128, [1, 1], scope='conv1x1')
        # conv11_1 = tf.nn.relu(conv11_1)
        # conv11_2 = slim.conv2d(conv11_1, 128, [3, 3], scope='conv3x3', padding='VALID')
        # conv10_1 = tf.nn.relu(conv11_2)
        end_points['block11'] = conv11_2
        print("conv11_2 shape = ", conv11_2.shape)
        # Prediction and localisations layers.
        predictions = []
        logits = []
        localisations = []      
    ...
    with tf.name_scope(scope, 'ssd_losses'):
        ...
        with tf.name_scope('cross_entropy_pos'):
            total_cross_pos = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=logits,
                                                                    labels=gclasses)
            total_cross_pos = tf.div(tf.reduce_sum(total_cross_pos * fpmask), batch_size, name='value')
            # total_cross_pos = tf.reduce_sum(total_cross_pos * fpmask, name="value")
            tf.losses.add_loss(total_cross_pos)

        with tf.name_scope('cross_entropy_neg'):
            total_cross_neg = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=logits,
                                                                    labels=no_classes)
            total_cross_neg = tf.div(tf.reduce_sum(total_cross_neg * fnmask), batch_size, name="value")
            # total_cross_neg = tf.reduce_sum(total_cross_neg * fnmask, name="value")
        ...
        with tf.name_scope('localization'):
            ...
            total_loc = tf.div(tf.reduce_sum(total_loc * weights), batch_size, name="value")
            # total_loc = tf.reduce_sum(total_loc * weights, name="value")
...
#         return total_loss
def ssd_losses_old1(logits, localisations,
               gclasses, glocalisations, gscores,
               match_threshold=0.5,
               negative_ratio=3.,
               alpha=1.,
...
def ssd_losses_old3(logits, localisations,
                   gclasses, glocalisations, gscores,
                   match_threshold=0.5,
                   negative_ratio=3.,
                   alpha=1.,
            ...
            for i in range(len(logits)):
                ...
                with tf.name_scope('block_%i' % i):
                    ...
                    with tf.name_scope('cross_entropy_pos'):
                        ...
                        loss = tf.div(tf.losses.compute_weighted_loss(loss, fpmask), n_positives, name="value")
                    ...
                    with tf.name_scope('cross_entropy_neg'):
                        ...
                        loss = tf.div(tf.losses.compute_weighted_loss(loss, fnmask), n_negative, name="value")
                    ...
                    with tf.name_scope('localization'):
                        ...
                        loss = tf.div(tf.losses.compute_weighted_loss(loss, weights), n_positives, name="value")
                        l_loc.append(loss)
